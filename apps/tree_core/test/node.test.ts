import { Node } from "../src/nodeContent"
...
describe("Node testing", () => {
    ...
    it("should create a sibling node and merge them", async () => {
        ...
        console.log(node2)
        const mergedNode = await Node.merge(node1, node2)
        console.log(mergedNode)
        expect(mergedNode.liabilties).toBe(96n)
        expect(mergedNode.rangeProof).toBeDefined()
    })
    it("should merge with a padding node", async () => {
        const mergedNode = await Node.merge(node1, paddingNode)
        console.log(mergedNode)
        expect(mergedNode.liabilties).toBe(6n)
        expect(mergedNode.rangeProof).toBeDefined()
    })
})